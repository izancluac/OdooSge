...
class BibliotecaNegocio(models.Model):
    _name = 'biblioteca.negocio'
    _description = 'Negocio relacionado con la biblioteca'
...
class ComicPrestamo(models.Model):
    _name = 'biblioteca.prestamo'
    _description = 'Prestamo de Còmic'

    comic_id = fields.Many2one('biblioteca.comic', string="Còmic", required=True)
    user_id = fields.Many2one('res.users', string="Usuari", required=True)
    start_date = fields.Date(string="Data d'inici", required=True, default=fields.Date.today)
    return_date = fields.Date(string="Data de devolució", required=True)

    @api.constrains('start_date')
    def _check_start_date(self):
        for record in self:
            if record.start_date > fields.Date.today():
                raise ValueError("La data d'inici no pot ser posterior al dia d'avui.")
    
    @api.constrains('return_date')
    def _check_return_date(self):
        for record in self:
            if record.return_date < fields.Date.today():
                raise ValueError("La data prevista de devolució no pot ser anterior al dia d'avui.")